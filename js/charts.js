...
import { fmt, pctFr, themeColor, causeColors, cssColor, el, viewEl } from './utils.js';
...
export function renderEvolutionCharts(viewId, entry, level, compareCodes) {
  ...
  function deltaPlugin(vals) {
    return {
      ...
      afterDraw: function(chart) {
        ...
        var sign = pct > 0 ? '+' : '';
        var color = pct > 0 ? themeColor('--danger') : pct < 0 ? themeColor('--safe') : themeColor('--c-muted');
        var ctx = chart.ctx;
        ctx.save();
  ...
  var evColors = barColors(themeColor('--c-1'), nYears);
  var ifColors = barColors(themeColor('--c-2'), nYears);
  ...
  var evCurColor = compareCodes.length ? themeColor('--accent') : themeColor('--c-1');
  var evDatasets = [{
    ...
    fill: !compareCodes.length, backgroundColor: themeColor('--accent-glow'), tension: 0.3,
  }];
  if (comparingEvo) {
  ...
  vs.evoCharts.push(new Chart(evCanvas, {
    ...
    options: Object.assign({}, baseOpts, {
      plugins: Object.assign({}, baseOpts.plugins, {
        title: { display: true, text: eventLabel, font: { family: 'var(--sans)', size: 13, weight: '500' }, color: themeColor('--text-secondary'), padding: { bottom: 8 } },
      }),
    }),
  }));
  ...
  if (ifCanvas) {
    ...
    var curColor = compareCodes.length ? themeColor('--accent') : themeColor('--c-2');
    var ifDatasets = [
      {
        ...
        fill: true, backgroundColor: compareCodes.length ? 'transparent' : themeColor('--accent-glow'), tension: 0.3,
      },
      {
        label: 'National', data: natIF,
        borderColor: themeColor('--c-muted'), borderWidth: 2, borderDash: [6, 3],
        pointRadius: 4, pointBackgroundColor: themeColor('--c-muted'),
    ...
    vs.evoCharts.push(new Chart(ifCanvas, {
      ...
      options: Object.assign({}, baseOpts, {
        plugins: Object.assign({}, baseOpts.plugins, {
          title: { display: true, text: 'Indice de Fréquence', font: { family: 'var(--sans)', size: 13, weight: '500' }, color: themeColor('--text-secondary'), padding: { bottom: 8 } },
        }),
      }),
    }));
  }
...
export function renderDemographics(viewId, sectors) {
  ...
  vs.demoCharts.push(new Chart(sexCanvas, {
    ...
    data: {
      ...
      datasets: [{
        data: sexValues,
        backgroundColor: [themeColor('--c-4'), themeColor('--c-3')],
        borderColor: themeColor('--bg-elevated'),
        borderWidth: 2,
      }]
    },
  ...
  // Le canevas ne lit pas color-mix() : on dégrade l'opacité du bleu de la charte à la main.
  var ageHex = themeColor('--c-1');
  var ageRgb = [1, 3, 5].map(function(k) { return parseInt(ageHex.slice(k, k + 2), 16); }).join(',');
  var ageColors = ageGroups.map(function(g, i) {
    return 'rgba(' + ageRgb + ',' + (0.3 + i / (ageGroups.length - 1) * 0.6).toFixed(2) + ')';
  });
...
export function renderDimensionPanel(viewId, panelId, sectors) {
  ...
  if (sectors.length < 2) {
    ...
    metrics.forEach(function(m, i) {
      vs.injuryCharts.push(buildInjuryChart(canvases[i], m.title, m.bars(sectors[0].dims, sectors[0].entry), m.color));
    });
    return;
  }
  buildCompareColumns(grid, sectors, function(sec, idx, body) {
    ...
    var cv = body.querySelectorAll('canvas');
    metrics.forEach(function(m, i) {
      vs.injuryCharts.push(buildInjuryChart(cv[i], m.title, m.bars(sec.dims, sec.entry), m.color));
    });
  });
}
